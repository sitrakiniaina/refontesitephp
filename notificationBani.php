...
if($_COOKIE['id_user']){
    ...
    $mbreNewsletter = addslashes(htmlentities($resultat['news']));
        ...
        if($all_histoOw->banni == '1' && $all_histoOw->view_bani_notif == '0'  ){

                echo json_encode(new sendData('1',$mbrePrenom));
                $pdo->exec("UPDATE users SET view_bani_notif = 1 WHERE id = '".$_COOKIE['id_user']."'   ");

        }else if($all_histoOw->banni == '0' && $all_histoOw->view_bani_notif == '1'  ){

                echo json_encode(new sendData('2',$mbrePrenom));
                $pdo->exec("UPDATE users SET view_bani_notif = 0 WHERE id = '".$_COOKIE['id_user']."'   ");

        }else{
                echo json_encode(new sendData('3',$_COOKIE['id_user']));
        }
}
