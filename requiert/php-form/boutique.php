<?php
	if (!empty($_POST['commander'])) {
			...
			if (!empty($_POST['idBoutiqueMontant'])) {
				...
				if ($mbreEuros >= 0.00 && $mbreEuros >= $idBoutiqueMontant) {
					...
					if ($actifBoutique == 1) {
						...
						$redirectionLogin = url_site.'/payement.php'; 
		...
		else { $reponsError = 'Vous ne pouvez pas faire un virement par ce que votre profil n\'est pas encore valide .'; $button = 'Fermer'; }
	}
